<?php
session_start();
require_once '../../controller/controller_page_admin_update_article.php';
?>
                    ...
                    <?php foreach ($selectArticle as $value) { ?>
                        <div class="col-xl-3 col-lg-3 col-md-6 col-sm-12 col-12 mt-3 mb-3">
                            <div class="card" style="width: 15rem;">
                                <a href="page_article_info.php?id=<?= $value['id'] ?>"><img class="card-img-top" src="../../assets/images/imgArticles/<?= $value['sp_article_image'] ?>" alt="Card image cap"></a>
                                <div class="card-body text-center">
                                    <p class="card-title h5"><?= $value['sp_article_title'] ?></p>
                                    <p class="card-title"><?= $value['sp_article_description'] ?></p>
                                    <a href="page_update_article.php?id=<?= $value['id'] ?>"><i class="text-dark fas fa-cog fa-2x"></i></a>
                                </div>
                            </div>
                        </div>
                    <?php } ?>
                        ...
                        <nav class="mx-auto m-4" aria-label="...">
                            <ul class="pagination">
                                <?php if ($currentPage == 1) {
                                    ...
                                    <li class="page-item">
                                        <a class="page-link" href="page_admin_update_article.php?page=<?= (($currentPage - 1) < 1 ? 1 : $currentPage - 1) ?>">Précédent</a>
                                    </li>
                                <?php } ?>
                                <?php for ($i = 1; $i <= $nbPages; $i++) { ?>
                                    <li class="page-item"><a class="page-link" href="page_admin_update_article.php?page=<?= $i ?>"><?= $i ?></a></li>
                                <?php }
                                if ($currentPage == $nbPages) {
                                    ...
                                    <li class="page-item">
                                        <a class="page-link" href="page_admin_update_article.php?page=<?= (($currentPage + 1) > $nbPages ? $nbPages : $currentPage + 1) ?>">Suivant</a>
                                    </li>
                                <?php } ?>
                            </ul>
                        </nav>
